...
function getTableDefinitions() {
    return [
        ...
        // Receipts - Aligned with actual MySQL schema (INT AUTO_INCREMENT)
        'receipts' => 'id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, company_id INT NOT NULL, payment_id INT NOT NULL, invoice_id INT, receipt_number VARCHAR(100) NOT NULL, receipt_date DATE NOT NULL, receipt_type VARCHAR(50) DEFAULT "payment_against_invoice", total_amount DECIMAL(15,2) NOT NULL, excess_amount DECIMAL(15,2) DEFAULT 0, excess_handling VARCHAR(50) DEFAULT "pending", change_note_id INT, status VARCHAR(50) DEFAULT "finalized", void_reason VARCHAR(255), voided_by INT, voided_at TIMESTAMP NULL, notes TEXT, created_by INT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY unique_receipt_number (company_id, receipt_number), KEY idx_receipts_company_id (company_id), KEY idx_receipts_payment_id (payment_id), KEY idx_receipts_invoice_id (invoice_id), KEY idx_receipts_created_at (created_at DESC)',
        'receipt_items' => 'id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, receipt_id INT NOT NULL, product_id INT, description TEXT NOT NULL, quantity DECIMAL(10,3) NOT NULL, unit_price DECIMAL(15,2) NOT NULL, tax_percentage DECIMAL(5,2) DEFAULT 0, tax_amount DECIMAL(15,2) DEFAULT 0, tax_inclusive TINYINT DEFAULT 0, discount_before_vat DECIMAL(15,2) DEFAULT 0, tax_setting_id INT, line_total DECIMAL(15,2) NOT NULL, notes TEXT, sort_order INT DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, KEY idx_receipt_items_receipt_id (receipt_id), KEY idx_receipt_items_product_id (product_id), KEY idx_receipt_items_created_at (created_at DESC)',
